...
**目标**：根据你的工作场景，选择并配置合适的AI工具

**步骤**：

1. **评估你的需求**
   - 你主要写什么类型的代码？（前端/后端/全栈/数据科学）
   - 你更习惯用IDE还是终端？
   - 你是否需要翻墙才能访问国外服务？
...
| 如果你... | 推荐工具 | 理由 |
|:---|:---|:---|
| 主要写代码，想要IDE集成 | Cursor / Trae | 专为编程设计，体验流畅 |
| 喜欢命令行，需要处理复杂任务 | Claude Code | 自主执行能力强 |
| 需要全能型助手（代码+文档+分析） | ChatGPT / Claude | 综合能力最强 |
| 国内网络，不想翻墙 | Trae / 通义灵码 | 国内可用，中文友好 |
| 预算有限 | Trae / 通义灵码 | 免费或低价 |
...
**任务**：用RPCT框架写一个提示词，让AI帮你完成以下任务之一：

选项A：解释一个你一直没搞懂的技术概念（如闭包、Promise、事件循环等）
选项B：写一个你常用的工具函数（如日期格式化、防抖节流等）
选项C：帮你写一封工作邮件
