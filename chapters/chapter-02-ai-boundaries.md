...
**目标**：建立一套属于自己的AI生成代码审查流程

**背景**：小李的事故告诉我们，AI生成的代码必须经过审查。但"审查"不是泛泛地看一遍，而是有检查项、有重点、有方法的系统性检查。

**步骤**：

1. **建立安全审查项**

基于小李踩过的坑，以下是你必须检查的安全点：
...
找一段你最近用AI生成的代码，逐条对照上述清单检查。记录发现的问题，并修复。

### 实践2：测试AI的"幻觉"边界

**目标**：亲身体验AI的"过度自信"，建立警惕意识

**任务**：向AI提出以下3类问题，观察它的反应：

**类型A：明确有答案的技术问题**
- 示例："JavaScript中let和var的区别是什么？"
- 预期：AI应该给出准确答案

**类型B：尚无定论的开放问题**
- 示例："2026年最受欢迎的JavaScript框架是哪个？"
- 预期：AI可能会编造或给出不确定的答案

**类型C：你编造的虚假问题**
- 示例："请介绍《AI编程进阶指南》这本书的作者和核心观点"（假设这本书不存在）
- 预期：AI可能会"一本正经地胡说八道"

**记录你的发现**：
- AI在哪类问题上表现最好？
- AI在哪类问题上最容易出错？
- 它出错时的语气是确定的、犹豫的、还是承认不知道的？

### 实践3：对比不同工具处理同一任务

**目标**：了解不同AI工具的能力边界差异

**任务**：选择同一个编程任务（如"写一个带有错误处理的文件上传接口"），分别用以下工具尝试：

1. IDE插件类（如Cursor/Copilot/通义灵码）
2. 对话类（如ChatGPT/Claude）
3. Agent类（如Claude Code，如果可用）
...
- [ ] 整理你自己的AI代码安全审查清单（至少10项）
- [ ] 用审查清单检查一段AI生成的代码，记录发现的问题
- [ ] 测试AI的"幻觉"边界（编造至少1个不存在的问题）
- [ ] 对比2种以上AI工具处理同一任务的效果
- [ ] 写下你的3条AI使用原则（贴在显眼位置提醒自己）
- [ ] 向至少1位同事分享小李的事故教训
